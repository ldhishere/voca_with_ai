...
const WordList: React.FC<WordListProps> = ({ words, visibility, onToggleFavorite, onDelete, isFilteringFavorites }) => {
  if (words.length === 0) {
    return (
  ...
  return (
    <div className="overflow-x-auto rounded-[2rem] border border-slate-200 shadow-sm bg-white">
      <table className="min-w-full md:min-w-[700px] divide-y divide-slate-100">
        <thead className="bg-slate-50/50">
          <tr>
            <th scope="col" className="w-16 px-4 py-5"></th>
            <th scope="col" className="min-w-[150px] px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">
              {visibility.word ? '단어' : '단어 (숨김)'}
            </th>
            <th scope="col" className="min-w-[150px] px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">
              {visibility.meaning ? '뜻' : '뜻 (숨김)'}
            </th>
            <th scope="col" className="min-w-[300px] px-6 py-5 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">
              {visibility.example ? '활용 문장' : '활용 문장 (숨김)'}
            </th>
            <th scope="col" className="w-16 px-4 py-5"></th>
          </tr>
        </thead>
