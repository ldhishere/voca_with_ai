 ...
 */
export async function generate100Words(language: Language): Promise<Omit<WordEntry, 'id' | 'createdAt' | 'isFavorite'>[]> {
  const ai = getAI();
 ...
 */
export async function lookupWord(word: string, language: Language): Promise<{ entry: Partial<WordEntry>, sources: any[] }> {
  const ai = getAI();
